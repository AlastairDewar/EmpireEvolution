...
class Member extends Controller {
	...
	function dashboard()
	{
		...
		{			
			...
			$this->parser->parse('members/user_dashboard', $template_data);
	...
	function confirm($code)
	{
		...
		$this->load->view('members/confirmation_success');
	}
	...
	function login()
	{
		...
		{			
			...
			else
			{
				...
				{
					$this->load->view('members/initial_login');
				}
				else
				{
					redirect('/member/dashboard','refresh');
	...
	function overview()
	{
		...
		{		
			if($this->Member_model->is_administrator())
			{
				$template_data = array();
				$template_data['username'] = $this->Member_model->get_username();
				$template_data['members'] = $this->Member_model->get_all_members();
				$template_data['highlight'] = $this->uri->segment(3);
				$this->parser->parse('members/overview', $template_data);
		...
		{			
			...
			else
			{
				...
				$this->load->library('email');
				  ...
				  $message .= "<a href=\"".site_url()."/member/confirm/".$code."\">".site_url()."/members/confirm/".$code."\"</a>\r\n\r\n";
