...
class News extends Controller
{
  ...
  function view($uid)
  {
		if($uid != 0 && is_int($uid))
		{
			...
			}else{redirec('news/invalid');}
		}else{redirect('news');}
  }
  function latest_articles()
  {
	$template_data = $this->News_model->get_latest_articles(3);
	// TODO format the template data in News_model
	$this->parser->parse('news_main', $template_data);
  }
  function archive()
  {
		...
		if($this->Member_model->is_administrator()){
			...
			if($uid != 0 && is_int($uid)){
					...
					// TODO Check its not already deleted
						...
						if ($this->form_validation->run() == FALSE)
						{
							$this->parser->parse('news_delete',$template_data);
						}
						else
						{
							$this->News_model->delete($uid);
							$this->parser->parse('news_delete_success',$template_data);
						}
					}else{redirect('news/invalid','refresh');}
